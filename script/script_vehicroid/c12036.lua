...
function s.cfilter(c)
	return c:IsSetCard(SET_ROID) and c:IsMonster() and c:IsAbleToGraveAsCost()
end
function s.thfilter(c)
	return (c:IsSetCard(SET_CYBERDARK) or c:IsSetCard(SET_ROID)) and c:IsMonster() and c:IsAbleToHand()
end
function s.check(sg,e,tp,mg)
	return sg:GetClassCount(Card.GetAttribute)==2 and sg:GetClassCount(Card.GetLocation)==2
		and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,sg)
end
...
function s.gyfilter(c)
	return c:IsSetCard(SET_ROID) and c:IsType(TYPE_FUSION) and c:IsAbleToGrave()
end
